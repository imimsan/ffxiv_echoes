...
public sealed class HelpTab : ITab
{
    ...
    public void Draw()
    {
        if (ImGui.BeginChild("##help-scroll", Vector2.Zero, true))
        {
            ...
            ImGui.TextWrapped(
                "1. 録画する：/echoes record on で録画モードを ON にし、対象コンテンツに突入してボスを 1 周見る\n" +
                "2. 録画から作る：/echoes の「コンテンツ一覧」で録画されたゾーンを開き、" +
                "「集計（観測イベント）」から欲しいキャストをトリガー化する\n" +
                "3. 再突入で確認：同じコンテンツに入り直し、該当キャストで音声・テキスト・俯瞰図などが出るか確認する");
            ImGui.Spacing();
            ...
            ImGui.TextWrapped(
                ...
                "・オーバーレイが見えない → 「全体設定」の「TTS のみのトリガーで自動オーバーレイ」を必要に応じて ON にする\n" +
